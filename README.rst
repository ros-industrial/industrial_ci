...
* `QUIET_CATKIN_COMMANDS` (default: not set): If set, suppress the print output of some `catkin {build, run_tests}` commands except for errors.
* `QUIET_SLEEP_LENGTH` (default: 60): Second to interval between print. Used only when `QUIET_CATKIN_COMMANDS` is set.
