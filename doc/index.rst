...
* **CLANG_TIDY_BASE_REF** (default: not set.): If set, clang-tidy tests on pull requests will only test packages with changes since the git base reference and all packages if HEAD == base ref.  This is useful on large projects because it reduces the time for CI to run on pull requests.
