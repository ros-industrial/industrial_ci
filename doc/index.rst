...
* **ROSDEP_OPTIONS** (default: -q): Options for ``rosdep install`` command, which you can see by ``rosdep install -h``. NOTE-1. the command is run by default with the following options and values: ``--from-paths $CATKIN_WORKSPACE/src --ignore-src --rosdistro $ROS_DISTRO -y``. NOTE-2. Since the default value is already set, be sure to re-add it if you'd like to keep it.
