...
* **CLANGCHECK** (default: not set. Value range: [<format-style>|false]): If set or a .clang-format file is in the repository root, run the clang-format check. If argument is not conform to `<format-style> <https://bcain-llvm.readthedocs.io/projects/clang/en/release_38/ClangFormatStyleOptions/#configurable-format-style-options>`_ use the default style. Setting the argument to false will ignore an existing .clang-format file.
