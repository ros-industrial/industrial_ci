...
* **BUILD_PKGS_WHITELIST** (default: not set): Packages to be built and tested can be explicitly specified with this, in ROS package name format (i.e. using underscore. No hyphen). Delimit with space (thus enclose the entire list by e.g. double quote). This can be used to reciprocally specify the packages that you don't want to be built nor tested (those not specified here won't be built nor tested). The pkgs listed here are prioritized than what's in ``TARGET_PKGS``.
...
* **PKGS_DOWNSTREAM** (default: explained): Packages in downstream to be built and tested. By default, packages listed in `BUILD_PKGS_WHITELIST` are used if anything, if not then those listed in `TARGET_PKGS` are used.
