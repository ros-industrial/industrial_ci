...
* **CLANG_FORMAT_CHECK** (default: not set. Value range: [<format-style>|file]): If set run the clang-format check. Set the argument to file to load style configuration from .clang-format file in the repository root. Alternatively, the argument can set he `<format-style> <https://bcain-llvm.readthedocs.io/projects/clang/en/release_38/ClangFormatStyleOptions/#configurable-format-style-options>`_. If not conform, the default style is used.
