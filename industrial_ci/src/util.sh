...
function ici_time_end {
    ...
    if [ -z "$ICI_START_TIME" ]; then
        ici_warn "[ici_time_end] var ICI_START_TIME is not set. You need to call ici_time_start in advance. Returning."
        return
    fi
    local end_time
    end_time=$(date -u +%s%N)
    local elapsed_seconds
    elapsed_seconds=$(((end_time - ICI_START_TIME) / 1000000000))
